...
### 序列化的核心规则
并非所有数据都能被序列化，Next.js（基于 React 和 JSON 规范）要求传递的数据必须是可序列化的数据类型，否则会报错。具体规则如下：
1. 支持的可序列化类型
- 基本类型：string、number、boolean、null
- 符合 JSON 规范的对象：{ key: value }（value 必须是可序列化类型）
- 符合 JSON 规范的数组：[value1, value2]（元素必须是可序列化类型）
- 日期对象（Date）：会被转换为 ISO 字符串（如"2024-05-20T12:00:00.000Z"），客户端可通过new Date(isoString)恢复
2. 不支持的不可序列化类型
- 函数（function）：无法转换为 JSON 字符串
- 正则对象（RegExp）：JSON 不支持
- 循环引用对象：const a = {}; a.b = a;（JSON 序列化会报错）
- 二进制数据（Buffer、ArrayBuffer）：JSON 无法直接表示
- 特殊对象：如Map、Set、Error（默认序列化结果不符合预期）
